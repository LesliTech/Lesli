...
RSpec.describe "GET:/administration/account/cronos", type: :request do 
    ...
    it "is expected to respond with a list of all cronos saved in the DB" do 
        ...
        # expect(@response_body_data.last["name"]).to be_a(Numeric)
        # expect(@response_body_data.last["name"]).to eql(cronos[:name])

        # expect(@response_body_data.last["description"]).to be_a(Numeric)
        # expect(@response_body_data.last["description"]).to eql(cronos[:description])
        ...
        # expect(@response_body_data.last["minute"]).to eql(cronos[:minute])

        # expect(@response_body_data.last["hour"]).to be_a(Numeric)
        # expect(@response_body_data.last["hour"]).to eql(cronos[:hour])

        # expect(@response_body_data.last["day_of_month"]).to be_a(Numeric)
        # expect(@response_body_data.last["day_of_month"]).to eql(cronos[:day_of_month])

        # expect(@response_body_data.last["month"]).to be_a(Numeric)
        # expect(@response_body_data.last["month"]).to eql(cronos[:month])

        # expect(@response_body_data.last["day_of_week"]).to be_a(Numeric)
        # expect(@response_body_data.last["day_of_week"]).to eql(cronos[:day_of_week])
