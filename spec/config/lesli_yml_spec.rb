...
RSpec.describe "lesli_settings.yml" do
    ...
    it "expect to have a section for configuration security settings" do
        ...
        expect(@lesli_settings["configuration"]["security"]).to have_key("allow_registration") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("allow_invitation") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("enable_websockets") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("enable_login_link") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("enable_login_otp") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("enable_becoming") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("enable_debug") 
        expect(@lesli_settings["configuration"]["security"]).to have_key("log_activity") 

        expect(@lesli_settings["configuration"]["security"]["allow_multiaccount"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["allow_registration"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["allow_invitation"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["enable_websockets"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["enable_login_link"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["enable_login_otp"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["enable_becoming"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["enable_debug"]).to be_in([true, false])
        expect(@lesli_settings["configuration"]["security"]["log_activity"]).to be_in([true, false])

        expect(@lesli_settings["configuration"]["security"]).to have_key("roles") 
        expect(@lesli_settings["configuration"]["security"]["roles"]).to be_an(Array) 
        expect(@lesli_settings["configuration"]["security"]["roles"].size).to be >= (1)
