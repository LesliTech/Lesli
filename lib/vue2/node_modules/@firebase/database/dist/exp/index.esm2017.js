import { _getProvider, _registerComponent, registerVersion } from '@firebase/app';
import { Component } from '@firebase/component';
import { stringify, jsonEval, contains, assert, base64, stringToByteArray, Sha1, isNodeSdk, assertionError, safeGet, map, stringLength, deepCopy, isMobileCordova, base64Encode, Deferred, isAdmin, isValidFormat, isEmpty, isReactNative, querystring, errorPrefix, validateArgCount, validateCallback, validateContextObject } from '@firebase/util';
...
class SyncPoint {
     ...
     */
    getView(query, writesCache, serverCache, serverCacheComplete) {
        ...
        if (!view) {
            ...
            const viewCache = new ViewCache(new CacheNode(eventCache, eventCacheComplete, false), new CacheNode(serverCache, serverCacheComplete, false));
    ...
    addEventRegistration(query, eventRegistration, writesCache, serverCache, serverCacheComplete) {
        const view = this.getView(query, writesCache, serverCache, serverCacheComplete);
...
class WriteTree {
        ...
        if (!removedWriteWasVisible) {
            ...
            if (writeToRemove.snap) {
                ...
                each(children, (childName) => {
                    this.visibleWrites_ = compoundWriteRemoveWrite(this.visibleWrites_, writeToRemove.path.child(childName));
                });
            }
            return true;
        }
    ...
    calcCompleteEventCache(treePath, completeServerCache, writeIdsToExclude, includeHiddenWrites) {
        if (!writeIdsToExclude && !includeHiddenWrites) {
            const shadowingNode = compoundWriteGetCompleteNode(this.visibleWrites_, treePath);
            if (shadowingNode != null) {
                return shadowingNode;
            }
            else {
                const subMerge = compoundWriteChildCompoundWrite(this.visibleWrites_, treePath);
            ...
            const merge = compoundWriteChildCompoundWrite(this.visibleWrites_, treePath);
    ...
    calcCompleteEventChildren(treePath, completeServerChildren) {
        ...
        const topLevelSet = compoundWriteGetCompleteNode(this.visibleWrites_, treePath);
        if (topLevelSet) {
    ...
    calcEventCacheAfterServerOverwrite(treePath, childPath, existingEventSnap, existingServerSnap) {
        ...
        if (compoundWriteHasCompleteWrite(this.visibleWrites_, path)) {
            ...
            const childMerge = compoundWriteChildCompoundWrite(this.visibleWrites_, path);
    ...
    calcCompleteChild(treePath, childKey, existingServerSnap) {
        ...
        const shadowingNode = compoundWriteGetCompleteNode(this.visibleWrites_, path);
        if (shadowingNode != null) {
            return shadowingNode;
        }
        else {
...
class SyncTree {
    ...
    getServerValue(query) {
        ...
        const serverCacheNode = serverCacheComplete
            ...
            : null;
        const writesCache = this.pendingWriteTree_.childWrites(query.path);
        const view = syncPoint.getView(query, writesCache, serverCacheComplete ? serverCacheNode.getNode() : ChildrenNode.EMPTY_NODE, serverCacheComplete);
...
class Repo {
    ...
    start() {
        ...
        this.statsReporter_ = StatsManager.getOrCreateReporter(this.repoInfo_, () => new StatsReporter(this.stats_, this.server_));
        // Used for .info.
     ...
     */
    getLatestState_(path, excludeSets) {
     ...
     */
    sendReadyTransactions_(node = this.transactionQueueTree_) {
     ...
     */
    sendTransactionQueue_(path, queue) {
     ...
     */
    rerunTransactions_(changedPath) {
     ...
     */
    rerunTransactionQueue_(queue, path) {
        if (queue.length === 0) {
     ...
     */
    getAncestorTransactionNode_(path) {
        let front;
     ...
     */
    buildTransactionQueue_(transactionNode) {
    ...
    aggregateTransactionQueuesForNode_(node, queue) {
     ...
     */
    pruneCompletedTransactionsBelowNode_(node) {
...
class RepoManager {
     ...
     */
    applyEmulatorSettings(repo, host, port) {
...
function registerDatabase() {
    _registerComponent(new Component('database-exp', (container, url) => {
