 ...
 */
function applyMutationToLocalView(mutation, maybeDoc, localWriteTime) {
    if (mutation instanceof SetMutation) {
...
function extractTransformMutationBaseValue(fieldTransforms, maybeDoc) {
    let baseObject = null;
...
function applySetMutationToRemoteDocument(mutation, maybeDoc, mutationResult) {
    ...
    if (mutationResult.transformResults) {
        ...
        newData = transformObject(mutation.fieldTransforms, newData, transformResults);
    }
    return new Document(mutation.key, mutationResult.version, newData, {
        hasCommittedMutations: true
    });
}
function applySetMutationToLocalView(mutation, maybeDoc, localWriteTime) {
    if (!preconditionIsValidForDocument(mutation.precondition, maybeDoc)) {
        return maybeDoc;
    }
    let newData = mutation.value;
    const transformResults = localTransformResults(mutation.fieldTransforms, localWriteTime, maybeDoc);
    newData = transformObject(mutation.fieldTransforms, newData, transformResults);
...
function applyPatchMutationToRemoteDocument(mutation, maybeDoc, mutationResult) {
    ...
    const transformResults = mutationResult.transformResults
        ? serverTransformResults(mutation.fieldTransforms, maybeDoc, mutationResult.transformResults)
        : [];
...
function applyPatchMutationToLocalView(mutation, maybeDoc, localWriteTime) {
    if (!preconditionIsValidForDocument(mutation.precondition, maybeDoc)) {
        return maybeDoc;
    }
    const version = getPostMutationVersion(maybeDoc);
 ...
 * holds.
 */
function patchDocument(mutation, maybeDoc, transformResults) {
    let data;
    if (maybeDoc instanceof Document) {
        data = maybeDoc.data();
    }
 ...
 */
function serverTransformResults(fieldTransforms, maybeDoc, serverTransformResults) {
 ...
 * @returns The transform results list.
 */
function localTransformResults(fieldTransforms, localWriteTime, maybeDoc) {
    const transformResults = [];
    for (const fieldTransform of fieldTransforms) {
        const transform = fieldTransform.transform;
        let previousValue = null;
...
function transformObject(fieldTransforms, data, transformResults) {
    const builder = new ObjectValueBuilder(data);
...
class MutationBatch {
    ...
    applyToLocalView(docKey, maybeDoc) {
        ...
        for (const mutation of this.baseMutations) {
            if (mutation.key.isEqual(docKey)) {
                maybeDoc = applyMutationToLocalView(mutation, maybeDoc, this.localWriteTime);
            }
        }
            ...
            return this.addMissingBaseDocuments(transaction, mutationBatches, results).next(mergedDocuments => {
                ...
                for (const batch of mutationBatches) {
                    for (const mutation of batch.mutations) {
                        ...
                        const mutatedDoc = applyMutationToLocalView(mutation, baseDoc, batch.localWriteTime);
...
function getDocFromCache(reference) {
    ...
    return firestoreClientGetDocumentFromLocalCache(client, reference._key).then(doc => new DocumentSnapshot$1(firestore, userDataWriter, reference._key, doc, new SnapshotMetadata(doc instanceof Document ? doc.hasLocalMutations : false, 
 ...
 */
class FirestoreDataConverter extends Compat {
    constructor(_firestore, _userDataWriter, delegate) {
        super(delegate);
        this._firestore = _firestore;
        this._userDataWriter = _userDataWriter;
    }
...
class DocumentReference$1 extends Compat {
    ...
    withConverter(converter) {
        return new DocumentReference$1(this.firestore, this._delegate.withConverter(FirestoreDataConverter.getInstance(this.firestore, converter)));
    }
}
/**
...
class Query$1 extends Compat {
    ...
    withConverter(converter) {
        return new Query$1(this.firestore, this._delegate.withConverter(FirestoreDataConverter.getInstance(this.firestore, converter)));
    }
}
...
class CollectionReference$1 extends Query$1 {
    ...
    withConverter(converter) {
        return new CollectionReference$1(this.firestore, this._delegate.withConverter(FirestoreDataConverter.getInstance(this.firestore, converter)));
    }
}
...
export { ArrayUnionFieldValueImpl as A, Blob as B, Compat as C, DeleteFieldValueImpl as D, FieldPath$1 as F, GeoPoint as G, MemoryPersistenceProvider as M, NumericIncrementFieldValueImpl as N, Query$1 as Q, ServerTimestampFieldValueImpl as S, Timestamp as T, WriteBatch$1 as W, FieldPath as a, ArrayRemoveFieldValueImpl as b, Firestore as c, Transaction$3 as d, DocumentReference$1 as e, DocumentSnapshot$2 as f, QueryDocumentSnapshot$2 as g, QuerySnapshot$1 as h, CollectionReference$1 as i, CACHE_SIZE_UNLIMITED as j, FirebaseFirestore$1 as k, loadBundle as l, namedQuery as n, setLogLevel$1 as s };
//# sourceMappingURL=database-92a9bc9c.js.map
