...
import componentSvg from "LesliVue/layout/lsvg.vue"
...
// · Load specific components from oruga, 
// · we import only few components so we do not depend so much on a library
import { Table, Collapse, Icon } from "@oruga-ui/oruga-next"
...
export default (engine, routes=[], data={}) => {
    ...
    const router = createRouter({
        ...
        routes: routes.map(route => {
            ...
            route.path = base_path.concat("/development").concat(route.path)
            return route
        })
        /*
    ...
    app.component("lsvg", componentSvg)
    app.component("component-header", componentHeader)
    app.component("component-toolbar", componentToolbar)
