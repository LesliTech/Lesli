...
class LesliUrl {
    ...
    engine(engine_path = null, path, params) {
        
        this.#path = ""
        // if engine is not null, then use it as the base url
        // engine_path could be null when the engine is mounted on the root path.
        // this happens on builders engines like LesliCloud.
        if (engine_path) {
            this.#path = engine_path.concat("/")
        }

        this.#path = this.#path.concat(this.#parsePath(path, params))
        return this
    }
...
export function useUrl() {
    ...
    return {
        ...
        core(path, params={}) {
            return (new LesliUrl()).engine("", path, params)
        },
