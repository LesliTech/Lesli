...
class LesliUrl {
    ...
    engine(engine = null, path, params) {
        
        this.#path = ""
        // if engine is not null, then use it as the base url
        // engine could be null if some engine is mounted in the root path
        // this only happens with the builder engines
        if (engine) {
            this.#path = engine.concat("/")
        }
