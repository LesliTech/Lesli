...
export const useAnnouncements = defineStore("core.announcements", {
    ...
    actions: {
         ...
         */
        getAnnouncements() {
            this.http.get(this.url.bell('announcements/list')
            .filter({ base_path: [this.router.currentRoute.path]})
        ...
        closeAnnouncement(announcement){
            this.http.post(this.url.bell('announcements/:annoucement_id/users', announcement.id)
            ).then(() => {
