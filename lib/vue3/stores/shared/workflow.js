    ...
    actions: {
        ...
        */
        ...
        deleteStatus(statusId) {
            ...
            const url = this.url[this.cloudModule](`${this.cloudObject}/:workflowId/statuses/:statusId`, { 
                workflowId: this.workflow.id,
                statusId: statusId
            })
            
            this.http.delete(url).then(result => {
                let statusIndex = this.workflow.statuses.findIndex(s => s.id = statusId)
                this.workflow.statuses.splice(statusIndex, 1)
            }).catch(error => {
                this.msg.danger(I18n.t("core.shared.messages_danger_internal_error"))
            }).finally(()=>{
                this.status.loading = false
