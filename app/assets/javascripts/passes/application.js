...
;// CONCATENATED MODULE: ../../app/vue2/passes/app.js
/*
Copyright (c) 2020, all rights reserved.
...
vue_public({
  ...
  methods: {
    postPassNew: function postPassNew(event) {
      var _this = this;

      event.preventDefault();
      this.progress_bar_active = true;
      this.http.post("/pass", this.pass).then(function (result) {
        _this.progress_bar_active = false;

        if (result.successful) {
          _this.showNotification("Login link sent successfuly", "has-text-success");
        } else {
