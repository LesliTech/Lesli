...
/* harmony default export */ const vue_public = (function (app) {
  ...
  functions_document.ready(function () {
    ...
    if (true) {
      ...
      debug_browser_default().info("[BUILD] (core) 220623.0716 ");
    }
  });
});
...
;// CONCATENATED MODULE: ../../app/vue2/users/sessions.js
/*
Copyright (c) 2020, all rights reserved.
...
vue_public({
  ...
  methods: {
    build_redirect_url: function build_redirect_url() {
      var default_path = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      ...
      } catch (error) {} // redirect to the default path if: 
    ...
    postLogin: function postLogin(event) {
      ...
      this.http.post(this.url.to("/login"), data).then(function (response) {
        _this.progress_bar_active = false; // If login went successful, redirect to root

        if (response.successful) {
          _this.url.go(_this.build_redirect_url(response.data.default_path));
        ...
        if (!response.successful && response.error.details.reset_password_token) {
          _this.url.go("/password/edit?reset_password_token=".concat(response.error.details.reset_password_token));
        }
    ...
    postLoginWithMFA: function postLoginWithMFA() {
      var _this2 = this;

      var key = encodeURIComponent(new URLSearchParams(window.location.search).get("key"));
      ...
      var endpoint = this.url.to("/login?key=".concat(key));
      var data = {
        user: {
          mfa_token: this.sign_in.mfa_token
        }
      };
      this.http.post(endpoint, data).then(function (response) {
        if (!response.successful) {
          _this2.showNotification(response.error.message);

          return;
        }

        _this2.url.go(_this2.build_redirect_url(response.data.default_path));
      }).catch(function (error) {
        _this2.showNotification("Error");
      });
    },
